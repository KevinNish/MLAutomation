...
invoke-command admaster1.vdi.network -ScriptBlock {
    param($hostname)
     NEW-ADOrganizationalUnit “$hostname” –path “OU=Goldmine Subscriptions,OU=Hosted Companies,DC=VDI,DC=NETWORK” } -credential $credentials -ArgumentList $hostname

invoke-command admaster1.vdi.network -ScriptBlock {
    param($hostname)
     NEW-ADgroup “$hostname-remote” -groupscope Global –path “OU=$hostname,OU=Goldmine Subscriptions,OU=Hosted Companies,DC=VDI,DC=NETWORK” } -credential $credentials -ArgumentList $hostname

invoke-command admaster1.vdi.network -ScriptBlock {
    param($hostname)
    get-adcomputer $hostname | Move-ADObject -TargetPath “OU=$hostname,OU=Goldmine Subscriptions,OU=Hosted Companies,DC=VDI,DC=NETWORK” } -credential $credentials -ArgumentList $hostname
